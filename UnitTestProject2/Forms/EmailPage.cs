...
namespace TestWebProject.Forms
{
    public class EmailPage : BasePage
    {
        ...
        public void CreateADraft(string emailAddress, string emailSubject, string emailText)
        {
            waiter = new WebDriverWait(Browser.GetDriver(), TimeSpan.FromSeconds(10));
            this.toField.SendKeys(emailAddress);
            this.subjectField.SendKeys(emailSubject);
            Browser.GetDriver().SwitchTo().Frame(this.textFieldBodyFrame.GetElement());
            this.textField.SendKeys(emailText);
